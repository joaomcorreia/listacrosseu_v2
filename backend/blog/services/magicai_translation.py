...
def get_magicai_config() -> Tuple[str, str, str]:
    api_key = os.environ.get("MAGICAI_API_KEY")
    if not api_key:
        raise RuntimeError(
            "MAGICAI_API_KEY is not set. Set MAGICAI_API_KEY before running translations."
        )

    api_url = os.environ.get("MAGICAI_API_URL", "https://api.magicai.ai/v1/chat/completions")
    model = os.environ.get("MAGICAI_MODEL", "magicai-translate-1")
...
def translate_blog_fields(source: Dict[str, str], target_language: str) -> Dict[str, str]:
    api_key, api_url, model = get_magicai_config()
    ...
    try:
        ...
        raise RuntimeError(
            f"MagicAI response was not valid JSON. Response: {content}"
        ) from exc
