...
const defaultVisibility = { owner_name: false, email: false, phone: true, website: true, city: true, region: true };
const entitlementStatuses = new Set(['active', 'paid', 'subscription']);

function hasGeneratedWebsiteAccess(business: DashboardBusiness | null) {
  if (!business?.generated_website) return false;
  return ['trial', 'active'].includes(business.generated_website.trial?.status || '') || entitlementStatuses.has(business.generated_website.status);
}

function trialDaysRemaining(endsAt?: string | null) {
  if (!endsAt) return 0;
  return Math.max(0, Math.ceil((new Date(endsAt).getTime() - Date.now()) / 86400000));
}

function WebsiteTrialStatus({ trial }: { trial: { status?: string; started_at?: string | null; ends_at?: string | null } }) {
  const active = trial.status === 'trial' || trial.status === 'active';
  const [language, setLanguage] = useState('en');
  useEffect(() => { setLanguage(window.location.pathname.split('/').filter(Boolean)[0] || 'en'); }, []);
  return <div className={`rounded-xl p-4 ${active ? 'bg-emerald-50 text-emerald-950' : 'bg-slate-50 text-slate-800'}`}><p className="font-black">{active ? 'Website trial active' : trial.status === 'expired' ? 'Website trial expired' : 'Website preview ready'}</p>{active && <div className="mt-2 grid gap-1 text-sm sm:grid-cols-3"><span>Started: {formatLocalizedDate(trial.started_at, language)}</span><span>Ends: {formatLocalizedDate(trial.ends_at, language)}</span><span className="font-black">{trialDaysRemaining(trial.ends_at)} days remaining</span></div>}{trial.status === 'expired' && <p className="mt-1 text-sm">Your private website draft is preserved.</p>}</div>;
}
...
export default function DashboardPageClient({ lang, initialBusinessId, initialPanel = 'overview' }: { lang: string; initialBusinessId?: number; initialPanel?: DashboardPanel }) {
  ...
  useEffect(() => { if (selected) setForm({ ...selected, visibility: { ...defaultVisibility, ...selected.visibility } }); }, [selected]);
  ...
  async function publishClaimedListing() {
    ...
    if (response.ok) { setForm({ ...data, visibility: { ...defaultVisibility, ...data.visibility } }); setBusinesses((items) => items.map((item) => item.id === data.id ? { ...data, visibility: { ...defaultVisibility, ...data.visibility } } : item)); setMessage(wasPublished ? 'Changes published. The public listing is updated.' : 'Claimed Listing published.'); }
  ...
  async function startWebsiteTrial() {
    if (!form || trialStarting || !form.generated_website) return;
    setTrialStarting(true); setError('');
    const response = await fetch(`${PUBLIC_API_BASE_URL}/api/dashboard/businesses/${form.id}/website/trial/`, { method: 'POST', credentials: 'include', headers: { 'X-CSRFToken': csrfValue || csrfToken() } });
    const data = await response.json().catch(() => ({}));
    if (response.ok) {
      const updated = { ...form, generated_website: { ...(form.generated_website || {}), status: data.website?.status || 'draft', page_title: data.website?.page_title || form.name, target_location: data.website?.target_location || '', trial: data.website?.trial || form.generated_website.trial, website: data.website } } as DashboardBusiness;
      setForm(updated);
      setBusinesses((items) => items.map((item) => item.id === updated.id ? updated : item));
    } else setError(data.detail || 'Unable to start your website trial.');
    setTrialStarting(false);
  }
...
function ClaimedListingPanel(props: { form: DashboardBusiness | null; lang?: string; categoryOptions: Option[]; cityOptions: Option[]; updateField: (field: keyof DashboardBusiness, value: string) => void; updateVisibility: (field: string, value: boolean) => void; setForm: React.Dispatch<React.SetStateAction<DashboardBusiness | null>>; setBusinesses: React.Dispatch<React.SetStateAction<DashboardBusiness[]>>; saveChanges: () => void; publishChanges: () => void; saving: boolean; message: string; error: string; logoUploading: boolean; uploadLogo: (file: File | undefined) => void; removeLogo: () => void; uploadBackground: (file: File | undefined) => void; openListing: () => void; createPreview: () => void; startTrial: () => void; trialStarting: boolean }) {
  ...
  return <>{props.form && <><div className="mb-4 flex items-center justify-end gap-3">{hasPublishedClaimedListing(props.form) ? <button type="button" onClick={props.openListing} className="rounded border border-emerald-700 bg-white px-4 py-2 font-bold text-emerald-800">View live listing</button> : <span className="text-sm text-slate-500">Not published yet</span>}</div><PostcardEditor value={props.form} categories={props.categoryOptions} uploading={props.logoUploading} onLogoUpload={props.uploadLogo} onBackgroundUpload={props.uploadBackground} showStyleControls onSave={props.saveChanges} onPublish={props.publishChanges} onRepublish={props.publishChanges} onUnpublish={() => togglePublication()} saving={props.saving} published={hasPublishedClaimedListing(props.form)} onChange={(field, value) => props.form && props.setForm({ ...props.form, ...(field === 'visibility' ? { visibility: value as Record<string, boolean> } : { [field]: value }) } as DashboardBusiness)} /><GeneratedWebsitePreviewSection business={props.form} lang={props.lang} createPreview={props.createPreview} startTrial={props.startTrial} creating={props.trialStarting} /></>}{props.form?.pending_category_suggestion && <p className="mt-4 rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Pending category suggestion: {props.form.pending_category_suggestion}. It is awaiting review and is not yet a canonical category.</p>}</>;
}
