...
import GeneratedWebsiteContactForm from './GeneratedWebsiteContactForm';
import GeneratedWebsiteSettingsPanel from './GeneratedWebsiteSettingsPanel';
import GeneratedHeroSlider from './GeneratedHeroSlider';
import ClassicBusinessVariantSelector from './ClassicBusinessVariantSelector';
import { getLocalizedWebsiteView, localizedContentFor, normalizeWebsiteLanguageConfig, SUPPORTED_GENERATED_WEBSITE_LANGUAGES, type GeneratedWebsite, type GeneratedWebsitePage, type SupportedLang } from './generated-page-schema';
import { CLASSIC_BUSINESS_VARIANTS, resolveGeneratedTemplateVariant } from './generated-page-registry';
...
const SOCIAL_NETWORKS = ['facebook', 'instagram', 'linkedin', 'tiktok', 'youtube', 'x', 'whatsapp'] as const;
...
export default function GeneratedWebsiteTemplate({ initial, businessId, lang, readOnly = false, privatePreview = false, activePage = 'home' }: { initial: GeneratedWebsite; businessId: string; lang: string; readOnly?: boolean; privatePreview?: boolean; activePage?: GeneratedWebsitePage }) {
  ...
  function updateWebsiteConfig(field: 'social_links' | 'location' | 'contact_form', value: unknown) {
  ...
  async function saveDraft() {
    ...
    const response = await fetch(`${PUBLIC_API_BASE_URL}/api/dashboard/businesses/${businessId}/website/`, {
      ...
      body: JSON.stringify({ template_id: draft.template_id === 'classic-business' ? 'classic-business' : 'editorial-v1', template_variant: classicVariant, target_location: draft.website.target_location, target_city: draft.website.target_city, target_region: draft.website.target_region, target_country: draft.website.target_country, service_area: draft.website.service_area, layout_mode: draft.website.layout_mode, theme: draft.website.theme, effects: draft.website.effects, settings: draft.website.settings, contact: draft.website.contact, language_config: draft.website.language_config, localized: draft.website.localized, social_links: draft.website.social_links, location: draft.website.location, contact_form: draft.website.contact_form }),
    });
  ...
  const heroSlides = sections.hero.slides?.length ? sections.hero.slides : [{ enabled: true, title: sections.hero.title, tagline: sections.hero.tagline, cta_label: sections.hero.cta_label, order: 0 }];
  const location = site.location || { address: [sections.contact.address, sections.contact.city, sections.contact.region, sections.contact.country].filter(Boolean).join(', '), latitude: null, longitude: null, map_enabled: true, directions_enabled: true };
  const mapQuery = location.latitude != null && location.longitude != null ? `${location.latitude},${location.longitude}` : location.address;
  const directionsUrl = `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(mapQuery || addressValue)}`;
  const previewToken = draft.preview_url ? new URL(draft.preview_url, 'http://local.invalid').searchParams.get('preview_token') || undefined : undefined;
  const page = activePage;
  const pageHref = (nextPage: GeneratedWebsitePage) => privatePreview ? `/${lang}/premium-preview?business=${businessId}&page=${nextPage}` : `/${lang}/generated/${site.website_slug}${nextPage === 'home' ? '' : `/${nextPage}`}`;
  const pageLinks: Array<{ id: GeneratedWebsitePage; label: string; available: boolean }> = [
    { id: 'home', label: 'Home', available: true },
    { id: 'about', label: 'About', available: privatePreview || Boolean(sections.about.enabled && (sections.about.intro || sections.about.text)) },
    { id: 'services', label: 'Services', available: privatePreview || Boolean(sections.services.enabled && services.length) },
    { id: 'gallery', label: 'Gallery', available: privatePreview || Boolean(gallery.enabled && gallery.items.some(Boolean)) },
    { id: 'faq', label: 'FAQ', available: privatePreview || publicFaqs.length > 0 },
    { id: 'contact', label: 'Contact', available: true },
  ];
  const socialLinkElements = SOCIAL_NETWORKS.filter((network) => Boolean(socialLinks[network])).map((network) => <a key={network} href={socialLinks[network]} target="_blank" rel="noreferrer" className="rounded-full border border-slate-200 px-3 py-1.5 text-xs font-bold capitalize text-slate-700 hover:border-slate-400">{network === 'x' ? 'X' : network}</a>);
  const classicVariant = resolveGeneratedTemplateVariant(draft.template_variant);
  const classicLayout = CLASSIC_BUSINESS_VARIANTS[classicVariant];
  ...
  return <div data-generated-page={page} data-classic-variant={classicVariant} data-classic-layout={classicLayout.description} className={`min-h-screen overflow-x-hidden bg-white text-slate-900 ${site.effects?.reveal ? 'generated-page-reveal' : ''}`} onClick={() => setActiveField(null)}>
    {!readOnly && <ClassicBusinessVariantSelector selected={classicVariant} primary={primary} onSelect={(next) => { setDraft((current) => ({ ...current, template_id: 'classic-business', template_variant: next })); markDirty(); }} />}
    ...
    <nav aria-label="Generated website pages" className="border-b border-slate-200 bg-white px-5 py-2"><div className="mx-auto flex max-w-7xl gap-4 overflow-x-auto text-xs font-bold text-slate-600">{pageLinks.filter((item) => item.available).map((item) => <a key={item.id} href={pageHref(item.id)} aria-current={page === item.id ? 'page' : undefined} className={`whitespace-nowrap rounded-full px-3 py-1.5 ${page === item.id ? 'bg-slate-900 text-white' : 'hover:bg-slate-100'}`}>{item.label}</a>)}</div></nav>
    {socialLinkElements.length > 0 && <div className="flex flex-wrap justify-end gap-2 border-b border-slate-100 bg-white px-5 py-2">{socialLinkElements}</div>}
    {editorEnabled && !readOnly && <GeneratedWebsiteSettingsPanel socialLinks={socialLinks} location={location} contactForm={site.contact_form || { enabled: true }} heroSlides={heroSlides} onSocialChange={(next) => updateWebsiteConfig('social_links', next)} onLocationChange={(next) => updateWebsiteConfig('location', next)} onContactFormChange={(next) => updateWebsiteConfig('contact_form', next)} onSlidesChange={(next) => updateSection('hero', { slides: next })} />}
    ...
    <main id="top">
      <section className="relative overflow-hidden px-5 py-24 text-white sm:py-32" style={{ backgroundColor: dark, backgroundImage: heroImage ? `linear-gradient(90deg, ${dark}${Math.round(heroOverlay * 255).toString(16).padStart(2, '0')}, ${dark}${Math.round(heroOverlay * 0.55 * 255).toString(16).padStart(2, '0')}), url(${heroImage})` : `linear-gradient(135deg, ${dark}, ${primary}99)`, backgroundPosition: heroImagePosition, backgroundSize: 'cover' }}>
        {classicVariant === 'variant-2' && heroImage && <div className="classic-variant-hero-image" aria-hidden="true" style={{ backgroundImage: `url(${heroImage})`, backgroundPosition: heroImagePosition }} />}
        <GeneratedHeroSlider slides={heroSlides} primary={primary} renderSlide={(slide, index) => <div className="generated-hero-slide mx-auto max-w-4xl text-center">{editable(`hero.slides.${index}.title`, slide.title || sections.hero.title, (value) => { const next = [...heroSlides]; next[index] = { ...next[index], title: value }; updateSection('hero', { slides: next }); }, 'mt-5 text-4xl font-black tracking-tight sm:text-6xl', 'h1')}{editable(`hero.slides.${index}.tagline`, slide.tagline || '', (value) => { const next = [...heroSlides]; next[index] = { ...next[index], tagline: value }; updateSection('hero', { slides: next }); }, 'mx-auto mt-6 max-w-2xl text-base leading-7 text-white/80 sm:text-lg', 'p', true)}{targetText && <p className="mx-auto mt-4 max-w-2xl text-sm font-semibold text-white/80">Serving {targetText}</p>}<a href={editorEnabled ? undefined : (slide.cta_href || '#contact')} onClick={(event) => { if (editorEnabled) event.preventDefault(); }} className="mt-8 inline-flex rounded-full px-6 py-3 text-sm font-bold text-white shadow-lg" style={{ backgroundColor: primary }}>{editable(`hero.slides.${index}.cta_label`, slide.cta_label || 'Get in touch', (value) => { const next = [...heroSlides]; next[index] = { ...next[index], cta_label: value }; updateSection('hero', { slides: next }); }, '', 'span')}</a></div>} />
        <div className="generated-hero-legacy mx-auto max-w-4xl text-center">
    ...
    {page === 'contact' && location.map_enabled && mapQuery && <section id="map" className="mx-auto max-w-7xl bg-white px-5 py-10"><div className="overflow-hidden rounded-2xl shadow-sm ring-1 ring-slate-200"><iframe title={`Map for ${draft.business_name}`} src={`https://www.google.com/maps?q=${encodeURIComponent(mapQuery)}&output=embed`} className="h-72 w-full border-0" loading="lazy" /></div>{location.directions_enabled && <a href={directionsUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white">Get directions</a>}</section>}
    {page === 'contact' && site.contact_form?.enabled !== false && <section id="contact-form" className="mx-auto max-w-4xl px-5 py-10"><h2 className="text-2xl font-black text-slate-900">Send a message</h2><GeneratedWebsiteContactForm slug={site.website_slug} previewToken={previewToken} /></section>}
    {socialLinkElements.length > 0 && <div className="flex flex-wrap gap-2 border-t border-white/10 px-5 py-4">{socialLinkElements}</div>}
    </main>
    ...
    {!readOnly && editorPanelOpen && <aside className="fixed bottom-4 right-4 z-30 max-h-[min(80vh,42rem)] w-[min(22rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
      ...
      {!privatePreview && <div className="mt-3 space-y-2"><button type="button" onClick={() => setNotice('Payment and publishing are not available yet.')} className="w-full rounded-lg px-3 py-2 text-sm font-bold text-white" style={{ backgroundColor: primary }}>Proceed to Payment and Publish</button><a href={`/${lang}/dashboard/generated-website?business=${businessId}`} className="block text-center text-xs font-bold text-slate-600">Back to Website Dashboard</a>{notice && <p className="text-center text-xs text-slate-600">{notice}</p>}</div>}
    </aside>}
  </div>;
}
