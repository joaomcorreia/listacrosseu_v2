...
export default function GeneratedWebsiteTemplate({ initial, businessId, lang }: { initial: GeneratedWebsite; businessId: string; lang: string }) {
  ...
  function updateTheme(key: 'primary' | 'dark', value: string) {
    ...
    if (key === 'primary') setPrimaryInput(normalized);
    else setDarkInput(normalized);
    setColorError('');
  }

  function updateCustomColor(key: 'primary' | 'dark', value: string) {
    if (key === 'primary') setPrimaryInput(value);
    else setDarkInput(value);
    const normalized = normalizeHex(value);
    if (!HEX_COLOR.test(normalized)) {
      setColorError('Enter a valid 6-digit color code, such as #2563eb.');
      return;
    }
  ...
  function resetSuggested() {
    updateTheme('primary', initial.website.theme.primary);
    updateTheme('dark', initial.website.theme.dark);
  }

  async function saveDraft() {
    setSaving(true); setNotice('');
    const response = await fetch(`${PUBLIC_API_BASE_URL}/api/dashboard/businesses/${businessId}/website/`, { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ theme: site.theme }) });
    setNotice(response.ok ? 'Website draft saved.' : 'Unable to save this draft.');
    setSaving(false);
  }
  ...
  async function startTrial() {
    ...
    if (response.ok) { setDraft(data); setNotice('Your 14-day free trial has started.'); } else setNotice(data.detail || 'Unable to start your trial.');
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main id="top">
      <section className="relative overflow-hidden px-5 py-24 text-white sm:py-32" style={{ backgroundColor: dark, backgroundImage: sections.hero.image ? `linear-gradient(90deg, ${dark}ee, ${dark}aa), url(${sections.hero.image})` : `linear-gradient(135deg, ${dark}, ${primary}99)` , backgroundPosition: 'center', backgroundSize: 'cover' }}><div className="mx-auto max-w-4xl text-center"><p className="text-xs font-bold uppercase tracking-[0.3em]" style={{ color: '#fde68a' }}>{sections.hero.title}</p><h1 className="mt-5 text-4xl font-black tracking-tight sm:text-6xl">{sections.hero.title}</h1>{sections.hero.tagline && <p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-white/80 sm:text-lg">{sections.hero.tagline}</p>}<a href="#contact" className="mt-8 inline-flex rounded-full px-6 py-3 text-sm font-bold text-white shadow-lg" style={{ backgroundColor: primary }}>Get in touch</a></div></section>
      {sections.services.enabled && services.length > 0 && <section id="services" className="px-5 py-20 sm:py-24"><SectionTitle eyebrow="What we offer" title="Services" primary={primary} /><div className="mx-auto mt-12 grid max-w-6xl gap-5 md:grid-cols-2 lg:grid-cols-3">{services.map((item, index) => { const service = typeof item === 'string' ? { name: item, description: '' } : item; return <article key={`${service.name}-${index}`} className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><div className="mb-5 flex h-11 w-11 items-center justify-center rounded-xl text-white" style={{ backgroundColor: primary }}>{index + 1}</div><h3 className="text-xl font-bold">{service.name || 'Service'}</h3>{service.description && <p className="mt-3 text-sm leading-6 text-slate-600">{service.description}</p>}</article>; })}</div></section>}
      {sections.about.enabled && <section id="about" className="px-5 py-20 text-white sm:py-24" style={{ backgroundColor: dark }}><div className="mx-auto grid max-w-6xl gap-10 md:grid-cols-2 md:items-center"><div className="rounded-3xl p-8" style={{ backgroundColor: `${primary}33` }}><p className="text-xs font-bold uppercase tracking-[0.25em]" style={{ color: '#fde68a' }}>About the business</p><h2 className="mt-4 text-3xl font-black">{sections.about.title}</h2><div className="mt-6 h-1 w-14 rounded" style={{ backgroundColor: primary }} /></div><p className="text-base leading-8 text-white/75">{sections.about.text || 'Add your business story in the Listing module.'}</p></div></section>}
