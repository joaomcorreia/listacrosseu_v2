...
export default function AccountVerificationClient({ lang }: { lang: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`${PUBLIC_API_BASE_URL}/api/account/verify/?token=${encodeURIComponent(token)}`, { credentials: 'include' })
      .then(async (response) => {
        ...
        setPasswordSetupRequired(Boolean(data.password_setup_required));
        setState('success');
      })
      .catch((error) => {
        setState('error');
  ...
  const next = businessId ? `/${lang}/dashboard?business=${businessId}${passwordSetupRequired ? '&setup=password' : ''}` : `/${lang}/dashboard${passwordSetupRequired ? '?setup=password' : ''}`;

  const continuation = authenticated ? next : `/${lang}/login?next=${encodeURIComponent(next)}`;
  const continuationLabel = authenticated && passwordSetupRequired ? 'Create your password' : authenticated ? 'Open my business dashboard' : 'Sign in to my business dashboard';
