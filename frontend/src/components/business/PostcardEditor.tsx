...
export default function PostcardEditor({ value, onChange, categories = [], cityOptions = [], onLogoUpload, onBackgroundUpload, onGalleryUpload, onGalleryRemove, allowMediaUpload = true, uploading = false, showStyleControls = true, showPreviewButton = true, canvasMode = 'editing', onSave, onPublish, onRepublish, onUnpublish, onViewLive, saving = false, published = false }: Props) {
  ...
  return <div className="space-y-4">
    ...
    {(onSave || onPublish || onRepublish || onUnpublish || onViewLive || showPreviewButton) && <div className="mx-auto flex max-w-5xl flex-wrap justify-end gap-2">{showPreviewButton && <button type="button" onClick={() => setPreviewOpen(true)} className="rounded border border-slate-300 bg-white px-4 py-3 font-bold text-slate-800">Preview</button>}{onSave && <button type="button" onClick={onSave} disabled={saving} className="rounded border border-blue-700 bg-white px-4 py-3 font-bold text-blue-700 disabled:opacity-60">{saving ? 'Saving draft...' : 'Save draft'}</button>}{!published && onPublish && <button type="button" onClick={onPublish} className="rounded bg-emerald-700 px-5 py-3 font-bold text-white">Publish</button>}{published && (onRepublish || onPublish) && <button type="button" onClick={onRepublish || onPublish} className="rounded bg-emerald-700 px-5 py-3 font-bold text-white">Publish changes</button>}{onViewLive && published && (value.canonical_path ? <a href={value.canonical_path} target="_blank" rel="noopener noreferrer" className="rounded border border-emerald-700 bg-white px-4 py-3 font-bold text-emerald-800">View live listing</a> : <button type="button" onClick={onViewLive} className="rounded border border-emerald-700 bg-white px-4 py-3 font-bold text-emerald-800">View live listing</button>)}{published && onUnpublish && <button type="button" onClick={onUnpublish} className="rounded border border-slate-400 bg-white px-5 py-3 font-bold text-slate-700">Unpublish</button>}</div>}
